...
//		}
    ...
    public function save($data = array()) {
        ...
        isset($data ['islock']) && $category_data ['islock'] = $data ['islock'];
        ...
        $menu_filedAvailable = array('name', 'ename', 'isshow'); //需要同步到menu的字段
        foreach ($category_data as $k => $v) {
            if (in_array($k, $menu_filedAvailable)) {
                $menu_data[$k] = $v;
            }
        }
        if ($category_data['parentid']>0) {//栏目修改上级同步到menu
            $menu_parentid = $_menu->field('menuid')->where("keyid = 'cat_{$category_data['parentid']}'")->find();
            $menu_parentid = $menu_parentid['menuid'];
            $menu_data['parentid'] = $menu_parentid;
        } else {
            $menu_data['parentid'] = 4;//内容发布管理，固定ID
        }
     ...
     * @return array
     */
    private function getClaChildrenIdArr($catid, $cattype = 'cat') {
        $ids = array();
        $catid = intval($catid);
        ...
        if ($parentid > 0) {
            ...
            if (!empty($data[0]['childrenidarr'])) {
                foreach ($data[0]['childrenidarr'] as $v) {
                    $datatmp = F('category_' . $v);
                    if ($datatmp['isshow'] == 1) {//判断是否显示
                        $data[] = $datatmp;
                    }
                }
            }
