...
//		dump($userData);dump($permissions);
	...
	public function add() {
		...
		if ($this->ispost()) {  //录入内容
			$shuxing_arr = $_POST['info']['shuxing'];
			if($shuxing_arr){
				sort($shuxing_arr);
				$shuxing_str = implode(',',$shuxing_arr);
			}
			if($shuxing_str){
				$in['info']['shuxing'] = $shuxing_str;
			}
			$_content = D ('Content');
			//令牌验证
		...
		if($in['catid']==2||$in['catid']==3||$in['catid']==4||$in['catid']==5){
			$_sz = M();
			...
			//获取所有服务商
			$_f = M();
			$sql = "select a.nickname,b.user_id from fangfa_user a
					left join fangfa_role_user b on a.user_id=b.user_id where a.isadmin=0 and b.role_id=3
					order by a.user_id desc";
			$fws_list = $_f->query($sql);
			//var_dump($fws_list);
...
//			if (!$_content->autoCheckToken($in)) $this->message('请不要非法提交或者重复刷新页面！');,由于tags参数不能传数组，请将数组参数格式化
		...
		$this->assign('form_data',$form);
		if($data['shuxing']){
			$data['shuxing_arr'] = explode(',',$data['shuxing']);
		}
		//var_dump($data['shuxing_arr']);
		$this->assign('data',$data);
		if($in['catid']==2||$in['catid']==3||$in['catid']==4||$in['catid']==5){
			$_sz = M();
			$sql = "select * from fangfa_shuxing order by sort asc limit 0,5";
