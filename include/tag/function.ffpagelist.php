...
function smarty_function_ffpagelist($params, &$smarty) {
    ...
    if (empty($data)) {
        ...
        $p = ($p ? $p : 1);
        $pagezie = $pagezie ? $pagesize : $smarty->_tpl_vars['pagesize'];
        //默认缩略图
        $default_thumb = (empty($default_thumb) ? '' : __ROOT__ . '/public/statics/' . $default_thumb);
	//如没有catid参数，获取当前页catid
        ...
        if (!isset($catid)) {
            ...
            if ($join) {
                ...
                if ($category_data['type'] != 'normal')
                    return '您要获取的栏目或子栏目没有内容模型，请刷新缓存试试！';
            ...
            //echo $_c->getLastSql();
            //构造内容栏目列表链接  $catid为多个的时候，取当前页
            //$pageurl = "__ROOT__/";
            if (isset($catids)) {
                $pageurl_root = substr($_SERVER['REDIRECT_URL'], 0, strrpos($_SERVER['REDIRECT_URL'], '/'));
                $pageurl .= rtrim($pageurl_root, '/') . '/index_{page}.html';
            } else {
            ...
            if (!empty($data['info'])) {
                foreach ($data['info'] as $k => $v) {
                    ...
                    if ($tlength)
                        $data['info'][$k]['title'] = msubstr($v['title'], 0, $tlength, $charset = "utf-8");
                    if ($dlength)
                        $data['info'][$k]['description'] = msubstr($v['description'], 0, $dlength);
                    if (empty($v['thumb']))
                        $data['info'][$k]['thumb'] = $default_thumb; //默认缩略图
	    	...
	    	//数据格式化
                if (!empty($tpl_id))
                    $smarty->_tpl_vars['__' . $tpl_id] = $data;
